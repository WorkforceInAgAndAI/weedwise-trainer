...
export default function HighSchoolHabitatConnect({ onComplete, onNext }: Props) {
  ...
  const getConnectionIndex = (weedId: string) => Object.keys(connections).indexOf(weedId);
  ...
  const handleRightClick = (habKey: string) => {
    ...
    setConnections(prev => ({ ...prev, [selectedLeft]: habKey }));
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 sm:p-6 space-y-4 animate-scale-in">
      <div>
        <h2 className="font-display font-bold text-lg text-foreground">🗺️ Habitat / Climate Connect</h2>
        <p className="text-sm text-muted-foreground">Match each species to its preferred habitat. Tap a weed on the left, then its habitat on the right.</p>
      </div>

      <div className="flex gap-4">
        {/* Left column - weeds with scientific names */}
              ...
              <button key={w.id} onClick={() => handleLeftClick(w.id)}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <span className="text-sm font-semibold text-primary italic block">{w.scientificName}</span>
                    <span className="text-[10px] text-muted-foreground">{w.commonName}</span>
                  </div>
                </div>
              </button>
            );
                ...
                <div className="flex items-center gap-2">
                  {isConnected && (
                    <div className="flex gap-0.5">
                      {Object.entries(connections).filter(([, v]) => v === hab.key).map(([k]) => (
      ...
      {checked && (
        <div className="rounded-lg p-4 space-y-3 animate-scale-in border border-border bg-muted/30">
            ...
            return (
              <div key={w.id} className={`flex items-center gap-3 p-2 rounded-lg ${isCorrect ? 'bg-accent/10' : 'bg-destructive/10'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-semibold text-primary italic">{w.scientificName}</span>
                  <span className="text-xs text-muted-foreground ml-1">({w.commonName})</span>
                  {!isCorrect && (
          ...
          <div className="bg-primary/10 rounded-lg p-3 text-xs text-foreground space-y-1">
            {picked.map(w => (
              <p key={w.id}><span className="font-semibold text-primary">💡 {w.scientificName}:</span> {w.memoryHook}</p>
            ))}
          </div>
