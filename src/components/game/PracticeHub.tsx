...
import SproutClimb from './practice-games/k5/SproutClimb';
...
const newK5Games: GameDef[] = [
 ...
 { id: 'fun-fact-detective', name: 'Fun Fact Detective', Icon: Search, category: 'Weed ID', description: 'Match each weed photo to its Fun Fact clue card from the 14 Weeds You Can Spot module!', howToPlay: 'Tap a clue card and then tap the weed photo you think it matches. Use the fun-fact hint (and the spot-it clue) to solve every case in the round.', component: FunFactDetective },
    ...
    { id: 'squad-defense', name: 'Squad Defense', Icon: ShieldAlert, category: 'Control Methods', description: 'Lane-defense action! Team up with the Weed Control Squad to stop marching weed villains before they crush the crops.', howToPlay: 'Weed villains march down 3 lanes toward your crops. Each weed shows a badge for the ONE superpower it fears. Tap a hero card (Pull, Block, Outsmart, Eat, or Stop), then tap the weed — matching hero = instant KO! Wrong hero still chips a little HP but wastes energy. Energy refills over time. Save the crops before HP hits zero!', component: SquadDefense },
    { id: 'sprout-climb', name: 'Sprout Climb', Icon: RefreshCw, category: 'Life Cycles', description: 'Chutes & Ladders style! Roll the dice and climb from seed to flower — grab resources to shoot up, dodge weeds that pull you back down.', howToPlay: 'Roll the dice to move your sprout up the board. Land on a 🌿 RESOURCE tile (water, sunlight, nutrients, healthy soil, pollinator) to CLIMB a vine to the next life stage. Land on a 🌾 WEED tile and slide down — real weeds steal what plants need! Move through Seed → Sprout → Leaf → Bud → Flower → new Seeds to complete the plant life cycle. Never touch a real weed unless a trusted adult says it\'s safe.', component: SproutClimb },
];
...
const highGames: GameDef[] = [
 ...
 { id: 'spot-differences', name: 'Flower Sex Detective', Icon: Search, category: 'Intra-species', description: 'Dioecious and monoecious weeds: match the clue to the male or female flowers.', howToPlay: 'Pick a section, read the trait clue, then tap the male or female image it describes.', component: SpotTheDifferences },
