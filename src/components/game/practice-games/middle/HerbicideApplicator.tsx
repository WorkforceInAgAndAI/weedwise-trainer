...
export default function HerbicideApplicator({ onBack }: { onBack: () => void }) {
 ...
 const applyHerbicide = () => {
  ...
  setRoundResults(prev => [...prev, { weedName: weed.weed.commonName, killed, resistanceGain }]);
 ...
 if (done) {
  ...
  return (
   <div className="fixed inset-0 bg-background z-50 flex flex-col items-center justify-center p-6">
    ...
    <LevelComplete level={level} score={totalScore} total={items.length * MAX_ROUNDS} onNextLevel={nextLevel} onStartOver={startOver} onBack={onBack} />
   </div>
  );
 }
 ...
 return (
  <div className="fixed inset-0 bg-background z-50 flex flex-col">
   ...
   {/* Main area: field left, sidebar right */}
    ...
    <div className="w-64 bg-card border-l border-border overflow-y-auto p-3 flex flex-col gap-3">
     {selectedWeed !== null ? (
      <>
       ...
       <p className="text-xs font-bold text-foreground">Herbicide MOA:</p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center gap-2">
       <p className="text-sm text-muted-foreground">Tap a weed in the field to select herbicide and rate.</p>
