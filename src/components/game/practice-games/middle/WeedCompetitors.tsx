...
function buildRound(you: typeof weeds[0], opponent: typeof weeds[0]) {
  const choices: { question: string; competitorInfo: string; options: Choice[] }[] = [
    {
...
export default function WeedCompetitors({ onBack }: { onBack: () => void }) {
  ...
  const restart = () => { setMatchIdx(0); setStep(0); setPoints(0); setTotalPoints(0); setPicked(null); setAnswered(false); setShowIntro(true); };
  ...
  // Match intro screen — brief description of both weeds
  if (showIntro && match) {
    return (
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center">
        {/* Match header — no gradient/shading */}
