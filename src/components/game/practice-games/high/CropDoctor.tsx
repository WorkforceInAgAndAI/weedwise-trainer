...
export default function CropDoctor({ onBack }: { onBack: () => void }) {
  ...
  const submit = (opt: string) => {
    ...
    setPicked(opt);
    setAnswered(true);
    if (opt === rounds[idx].correct) {
      setMoney(m => m + rounds[idx].reward);
      setCorrectCount(c => c + 1);
    }
  };
  const next = () => { setIdx(i => i + 1); setPicked(null); setAnswered(false); };
  const restart = () => { setIdx(0); setPicked(null); setAnswered(false); setMoney(0); setCorrectCount(0); };
  ...
  if (done) {
    ...
    return (
      <div className="fixed inset-0 bg-background z-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-foreground mb-2">Score: {correctCount}/{rounds.length}</p>
        <p className="text-muted-foreground mb-6">Earned: ${money.toLocaleString()}</p>
      ...
      <div className="max-w-lg mx-auto p-4">
        ...
        <div className="grid gap-2">
          {c.options.map(opt => {
            ...
            else if (answered && opt === picked) cls = 'border-red-500 bg-red-500/20';
            return (
