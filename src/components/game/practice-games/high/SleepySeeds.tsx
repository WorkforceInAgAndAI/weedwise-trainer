...
export default function SleepySeeds({ onBack }: { onBack: () => void }) {
  ...
  const rounds = useMemo(() => shuffle(SCENARIOS).slice(0, 10), []);
  // Pick a random weed for each round to use as seed ID
  const seedWeeds = useMemo(() => shuffle([...weeds]).slice(0, 10), []);

  const [idx, setIdx] = useState(0);
  // Phase: 'seedId' = identify the weed from seed, 'dormancy' = pick strategy
  ...
  // Build seed ID options for current round
  useMemo(() => {
  ...
  if (done) {
    const total = rounds.length * 2; // 2 points per round (seed ID + dormancy)
        ...
        <div className="max-w-lg mx-auto p-4">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {seedIdOptions.map(wId => {
              ...
              else if (wId === seedIdAnswer && !seedIdChecked) cls = 'border-primary bg-primary/10';
              return (
