...
export default function ControlMethodMatching({ onBack }: { onBack: () => void }) {
 ...
 const items = useMemo(() => {
  ...
  return pool.slice(offset).concat(pool).slice(0, 8).map(w => ({ weed: w, correct: getMOA(w) }));
 }, [level]);
 ...
 const submit = (gId: string) => {
  ...
  setPicked(gId);
  setAnswered(true);
  const correct = gId === items[idx].correct;
  if (correct) setScore(s => s + 1);
  setResults(prev => [...prev, { weed: items[idx].weed, correct, picked: gId, best: items[idx].correct }]);
 };
       ...
       {wrongResults.map((r, i) => (
        <div key={i} className="bg-card border border-border rounded-xl p-3 flex items-center gap-3">
         ...
         <div className="flex-1">
          ...
          <p className="text-xs text-destructive">You: {MOA_GROUPS.find(g => g.id === r.picked)?.label}</p>
          <p className="text-xs text-green-600">Correct: {MOA_GROUPS.find(g => g.id === r.best)?.label}</p>
         </div>
        </div>
       ))}
      </div>
