...
const CROP_LEVELS = [
 { id: 'rr-soy', name: 'Roundup Ready Soybean', compatible: ['gly', 'ppo', 'multi'], rotationValue: 0.4, bg: soyBg },
 { id: 'conv-corn', name: 'Conventional Corn', compatible: ['als', 'ppo', 'multi'], rotationValue: 0.7, bg: cornBg },
 { id: 'wheat', name: 'Winter Wheat', compatible: ['als', 'ppo', 'multi'], rotationValue: 0.8, bg: wheatBg },
];
...
function getYearExplanation(crop: string, herb: string, yearIdx: number, allChoices: { crop: string; herb: string }[]): string {
 ...
 const parts: string[] = [];
 if (!compatible) parts.push(`${herbDef?.name} is not compatible with ${cropDef?.name}. This would damage your crop.`);
 else parts.push(`${herbDef?.name} is compatible with ${cropDef?.name}.`);
 if (herbDef?.risk === 'high') {
  const prevSameHerb = allChoices.slice(0, yearIdx).filter(c => c.herb === herb).length;
  if (prevSameHerb > 0) parts.push(`You used this high-risk herbicide ${prevSameHerb + 1} times now. Repeated use greatly increases resistance.`);
  else parts.push(`This herbicide has high resistance risk. Avoid using it repeatedly.`);
 } else if (herb === 'multi') {
  parts.push(`Great choice! Multi-MOA mixes reduce selection pressure.`);
 } else {
  parts.push(`${herbDef?.name} has ${herbDef?.risk} resistance risk.`);
 }
 return parts.join(' ');
}
...
export default function HerbicideResistor({ onBack }: { onBack: () => void }) {
 ...
 // Each level uses a different crop
 const currentCrop = CROP_LEVELS[(level - 1) % CROP_LEVELS.length];

 // Different weeds per level
     ...
     <div className="text-left bg-secondary/50 rounded-xl p-4 mb-4 max-w-sm w-full space-y-3">
      {choices.map((c, i) => (
       <div key={i} className="border-b border-border pb-2 last:border-0">
        <p className="text-sm font-bold text-foreground">Year {i + 1}: {HERBICIDES.find(h => h.id === c.herb)?.name}</p>
  ...
  return (
   <div className="fixed inset-0 bg-background z-50 flex flex-col items-center justify-center p-6">
    <div className="max-w-md w-full bg-card rounded-xl border border-border p-6">
     ...
     <p className="text-sm text-foreground mb-1">{currentCrop.name} + {herbDef?.name}</p>
     {/* Show weed suppression */}
