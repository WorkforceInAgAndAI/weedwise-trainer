...
export default function AuthModal({ onClose, onAuthenticated, defaultMode = 'login' }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-xl shadow-lg max-w-md w-full p-6 space-y-5 animate-scale-in" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-display font-bold text-foreground">{modeTitle[mode]}</h2>
            ...
            <div className="border-t border-border pt-4 space-y-2">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => { setMode('signup-instructor'); setError(''); }}
                  className="flex-1 px-3 py-2 rounded-lg border border-primary/30 text-primary text-sm hover:bg-primary/10 transition-colors">
                  📊 Instructor
                </button>
                <button type="button" onClick={() => { setMode('signup-student'); setError(''); }}
                  className="flex-1 px-3 py-2 rounded-lg border border-accent/30 text-foreground text-sm hover:bg-accent/10 transition-colors">
                  🎮 Student
                </button>
              </div>
            </div>
