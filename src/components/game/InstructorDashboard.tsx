...
interface StudentRow { id: string; nickname: string; class_id: string; }
...
export default function InstructorDashboard({ onClose }: Props) {
  ...
  // Load classes belonging to this instructor
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={logout} className="px-4 py-2 rounded-lg border border-destructive/50 text-destructive hover:bg-destructive/10 transition-colors text-sm">
              Sign Out
            </button>
        ...
        {classes.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-sm text-muted-foreground">Create a class from the "Join / Create Class" button to start tracking students.</p>
          </div>
        ) : (
          <>
            ...
            {showQR && selectedClassInfo && (
              <div className="mb-4 p-6 bg-card border border-border rounded-xl text-center space-y-3">
                <p className="text-sm text-muted-foreground">Students can scan this QR code to join the class:</p>
            ...
            {tab === 'overview' && (
              <div className="space-y-6">
                ...
                {students.length === 0 && (
                  <p className="text-center text-muted-foreground py-8">No students have joined yet. Share the join code or QR code with your class!</p>
                )}
              </div>
            )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-muted">
                      {['Nickname','Sessions','XP','Correct','Wrong','Accuracy','Streak','Mastered','Badges','Last Played'].map(h => (
