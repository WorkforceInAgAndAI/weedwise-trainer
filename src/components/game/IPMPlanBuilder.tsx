...
function scoreMatch(weed: Weed, cultural: string, chemical: string, mechanical: string): { score: number; feedback: string[] } {
 ...
 else if (cultural !== 'None') { score += 10; feedback.push(' Cultural method partially helpful'); }
 ...
 else if (chemical !== 'None') { score += 10; feedback.push(' Chemical choice could be more targeted'); }
 ...
 else if (mechanical !== 'None') { score += 10; feedback.push(' Mechanical method somewhat useful'); }
 else { score += 5; feedback.push('ℹ Consider adding mechanical control'); }
...
export default function IPMPlanBuilder({ weed, onComplete, onNext }: Props) {
 ...
 <SelectCategory label=" Mechanical Control" options={MECHANICAL_OPTIONS} value={mechanical} onChange={setMechanical} disabled={submitted} />
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-1"><span className="text-primary"></span> {weed.controlTiming}</p>
 </div>
